...
class GameEngine {
  ...
  _emitRoomAmbient(room){
    ...
    if (this._roomHasActiveCombat(room)) return;
    const def = this._getDef(room.defId);
    const hasOthers = room.playerIds.length > 1;
    const line = hasOthers
      ? this._pickRandomFrom(def.crowdAmbientLines, room, "crowd") || this._pickRandomFrom(def.ambientLines, room, "ambient")
      : this._pickRandomFrom(def.ambientLines, room, "ambient");
    if (!line) return;
    room.ambientMeta.lastAmbientAt = U.now();
    this._emitEvent(Schema.EventKind.SYSTEM, line, null, { roomId: room.id }, "room");
  }
